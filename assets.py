...
HEADLESS_OPTIONS = ["--headless=new", "--disable-gpu", "--disable-dev-shm-usage","--window-size=1920,1080","--disable-search-engine-choice-screen","--disable-blink-features=AutomationControlled"]
...
GROQ_LLAMA_MODEL_FULLNAME="deepseek-r1-distill-llama-70b"
...
evaluation_message = f"""Tu agiras en tant qu'expert en évaluation de sites web et SEO. Ta mission est d'analyser un site web en fonction de son contenu, de son optimisation SEO et de son ergonomie. Tu utiliseras un ensemble de règles spécifiques stockées dans une variable sous format JSON pour réaliser cette analyse. Ce fichier JSON contient des critères de jugement détaillés pour différentes catégories, telles que la structure de la navigation, la qualité du contenu, l’expérience utilisateur (UX), le design, le SEO technique, le maillage de liens et l'accessibilité.
                        Le processus d'analyse doit inclure :
                        {rules}
                        Format attendu de la réponse :
                        Résumé global de l'analyse.
                        Liste des problèmes identifiés avec leur score et pénalité.
                        Recommandations détaillées pour chaque point à améliorer, en mettant l’accent sur les points les plus importants en termes d’impact SEO et d’ergonomie.
